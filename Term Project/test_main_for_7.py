...
def is_logo_candidate(contour, img):
    ...
    max_area = 0.3 * img.shape[0] * img.shape[1]
...
def segment_logo(image_path):
    ...
    for i, contour in enumerate(contours):
        if is_logo_candidate(contour, img):            # Create a mask from the contour
            ...
            cv2.imwrite(f'./testSegment/{image_name}_segmented_{i}.{image_file_type}', final_image)

# Example usage
imagePath = ['handbag7.jpg']
